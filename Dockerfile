# BentoPDF is built for the root of its own hostname, such as
# https://pdf.smartnetcafe.com. Keep BASE_URL blank in docker-compose.yml.
ARG BASE_URL=
...
# The app processes files in the browser and does not require build secrets.
# Keeping this as a normal RUN command makes `docker compose up --build` work
# with standard Docker/Compose without BuildKit secret configuration.
...
# This image is intentionally served from the hostname root.
