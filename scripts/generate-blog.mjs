...
function generate() {
  ...
  const authorPagePath = path.join(BLOG_DIR, 'author-alam.html');
  if (fs.existsSync(authorPagePath)) {
    const authorHtml = fs.readFileSync(authorPagePath, 'utf-8');
    fs.writeFileSync(
      authorPagePath,
      authorHtml.replaceAll('https://www.bentopdf.com', SITE_URL)
    );
  }
