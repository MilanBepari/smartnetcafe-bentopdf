...
const init = async () => {
  ...
  document.getElementById('faq-accordion')?.remove();
  document.getElementById('hero-section')?.remove();
  document.getElementById('used-by-section')?.remove();
  document.getElementById('features-section')?.remove();

  document.querySelectorAll('.faq-list').forEach((faqList) => {
    faqList.closest('section')?.remove();
  });
  document
    .querySelectorAll('#faq-open-btn, #faq-modal, #faq-close-btn')
