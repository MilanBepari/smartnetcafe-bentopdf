...
export const renderFileDisplay = (container: HTMLElement, files: File[]) => {
  ...
  if (files.length > 0) {
    files.forEach((file: File) => {
      ...
      fileDiv.className =
        'uploaded-file flex items-center justify-between bg-gray-700 p-3 rounded-lg text-sm';

      const nameSpan = document.createElement('span');
      nameSpan.className = 'uploaded-file-name truncate font-medium text-gray-200';
      nameSpan.textContent = file.name;

      const sizeSpan = document.createElement('span');
      sizeSpan.className = 'uploaded-file-size flex-shrink-0 ml-4 text-gray-400';
