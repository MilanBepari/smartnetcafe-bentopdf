...
function getBasePath(): string {
  return (process.env.BASE_PATH || process.env.BASE_URL || '/').replace(/\/$/, '');
}
      ...
      if (pageName && PAGES.has(pageName)) {
        if (isDev) {
          ...
          if (fs.existsSync(srcPath)) {
            req.url =
              `${basePath}/src/pages/${pageName}.html` +
...
function rewriteHtmlPathsPlugin(): Plugin {
  const baseUrl = process.env.BASE_PATH || process.env.BASE_URL || '/';
...
function stripFaqMarkupPlugin(): Plugin {
  const stripFaqMarkup = (html: string) =>
    html
      .replace(/\s*<!--\s*FAQ Section\s*-->[\s\S]*?<\/section>/gi, '')
      .replace(
        /\s*<section\b[^>]*\bid=["']faq-accordion["'][^>]*>[\s\S]*?<\/section>/gi,
        ''
      )
      .replace(
        /\s*<section\b[^>]*>[\s\S]*?data-i18n=["']faq\.sectionTitle["'][\s\S]*?<\/section>/gi,
        ''
      )
      .replace(
        /\s*<script\b[^>]*type=["']application\/ld\+json["'][^>]*>(?=[\s\S]*?"@type"\s*:\s*"FAQPage")[\s\S]*?<\/script>/gi,
        ''
      );

  return {
    name: 'strip-faq-markup',
    transformIndexHtml(html) {
      return stripFaqMarkup(html);
    },
    writeBundle(options, bundle) {
      const outDir = options.dir;
      if (!outDir) return;
      ...
      for (const fileName of Object.keys(bundle)) {
        ...
        if (!fs.existsSync(diskPath)) continue;
        const source = fs.readFileSync(diskPath, 'utf8');
        const updated = stripFaqMarkup(source);
        if (updated !== source) fs.writeFileSync(diskPath, updated);
      }
    },
  };
}
...
export default defineConfig(() => {
  ...
  return {
    base: (process.env.BASE_PATH || process.env.BASE_URL || '/').replace(/\/?$/, '/'),
    worker: {
      format: 'es' as const,
    },
    plugins: [
      ...
      handlebars({
        ...
        context: {
          baseUrl: (process.env.BASE_PATH || process.env.BASE_URL || '/').replace(/\/?$/, '/'),
